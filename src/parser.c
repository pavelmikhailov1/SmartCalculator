...
int parser(char *str, char *result) {
  ...
  // printf("%s\n", str_buf);
  if (*str_buf == '+' ||
      *str_buf == '-') {  //если строка начинается с унарного + или -
    result[0] = '0';
  }
  while (*str_buf) {
    if (error == INCORRECT_EXPRESSION) break;
    if (strchr(VALID_CHARACTERS, *str_buf) ==
        NULL) {  //проверка каждого символа на валидность
    ...
    if (isdigit(*str_buf) || *str_buf == '.') {  //если символ - цифра
      ...
      if (*str_buf == '(' && (*(str_buf + 1) == '+' || *(str_buf + 1) == '-') &&
          (isdigit(*(str_buf + 2)) ||
           strchr("sctbnvqzo", *(str_buf + 2)) !=
               NULL)) {  //если скобка а за ней унарный оператор(обработка
                         //краевой ситуации)
        strcat(result, "((0");
      ...
      result[strlen(result)] = get_character(
          &str_buf, &char_flag_err);  //помещаем символ в результирующую строку
...
int check_correct_string(
    char *str) {  //обработка возможных ошибок в строке после парсинга
  int status = OK;
  char *ptr = str;
  while (*ptr) {
    ...
    if ((strchr("+-*/^", *ptr) != NULL) &&
        (strchr("+-*/^", *(ptr + 1)) !=
         NULL)) {  //если в строке идут подряд два оператора
      status = INCORRECT_EXPRESSION;
    } else if (*ptr == ')' &&
               (strchr("+-*/^)m", *(ptr + 1)) == NULL &&
                *(ptr + 1) != '\0')) {  //после скобки должен быть оператор или
                                        //конец строки
      status = INCORRECT_EXPRESSION;
    } else if (*ptr == 'm' && (strchr("+-", *(ptr - 1)) != NULL ||
                               strchr("+-", *(ptr + 1)) !=
                                   NULL)) {  //между функцией мод обязанны быть
                                             //два числа или скобки или x
      status = INCORRECT_EXPRESSION;
...
char get_character(char **str, int *err) {
  ...
  if (strchr("+-*/^x", **str) != NULL) {  //если текущий символ оператор +-*/^x
    ...
    return tmp;
             ...
             NULL) {  //если текущий символ начало функции
...
char get_function(char **str) {
  char res = '!';
  if (strchr("cst", **str) != NULL &&
      *(*str + 1) != 'q') {  //если это тригонометрическая функция
...
double add_number_to_str(char **pointer) { return strtod(*pointer, pointer); }
