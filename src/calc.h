...
#define OPERAND 0 ///< флаг, который сообщает о том, что узел добавляется в стек операторов
#define NUMBER 1  ///< флаг, который сообщает о том, что узел добавляется в стек чисел

#define OK 0 ///< работа прошла без ошибок (возвращаемое значение)
#define INCORRECT_EXPRESSION 1 ///< работа завершилась с ошибкой "Не корректное выражение" (возвращаемое значение)
#define ERROR_CAlCULATION 2 ///< работа завершилась с ошибкой "Ошибка калькуляции" (возвращаемое значение)

#define EPS 10e-9 ///< Макрос для сравнения чисел типа double с точностью до 9 знаков после запятой
#define ERROR_CH '!' ///< Макрос, который сообщает, что в функциях парсинга при проверке строки на валидность была найдена ошибка

#define VALID_CHARACTERS "1234567890.()+-*/^modcsintaqrtlgx" ///< для функции парсинга, которая проверяет символы входной строки на валидность
#define BINARY_OPERATORS "+-*/^m" ///< для функции парсинга, которая ищет во входной строке бинарные операторы
#define FUNCTIONS "sctbnvqzo" ///< для функции парсинга, которая ищет во входной строке функции и заменяет их на соответствующие символы
...
/*!
  ...
  @param x Значение X
  @return Статус ошибки : 0 - работа завершилась корректно, 1 - работа завершилась с ошибкой "Не корректное выражение"
*/
...
/*!
  @brief Функция для парсинга входной строки, которая выполняет следующую работу:
  Проверка каждого символа строки на валидность;
  Определение чисел;
  Определение бинарных операторов;
  Определение унарных операторов;
  Определение функций с дальнейшей заменой их на соответствующие короткие обозначения(для удобства в дальнейшем формировании строки в обратной постфиксной записи).
  @param str Входная строка с выражением
...
/*!
  ...
  @param str Входная строка с выражением
  @return Специальный символ, полученный путем проверки строки на наличие корректно написанной функции
*/
...
/*!
  @brief Функция определяет что в строке находится обратная тригонометрическая функция
  @param str Входная строка с выражением
  @return Специальный символ, полученный путем проверки строки на наличие корректно написанной функции
*/
...
/*!
  @brief Функция определяет что в строке находятся функции десятичного или натурального логарифма
  @param str Входная строка с выражением
  @return Специальный символ, полученный путем проверки строки на наличие корректно написанной функции
*/
...
/*!
  @brief Функция определяет что в строке находится функция взятия остатка от деления
  @param str Входная строка с выражением
  @return Специальный символ, полученный путем проверки строки на наличие корректно написанной функции
*/
...
/*!
  @brief Функция определяет что в строке находятся число и переписывает его в выходную строку
  @param str Входная строка с выражением
...
/*!
  @brief Функция меняет в строке точки на запятые(особенности сборки проекта через cmake)
  @param str Входная строка с выражением
*/
...
/*!
  ...
  @param flag_head Флаг для определения под узел какого типа будет выделяться память и сколько ее выделить
...
/*!
  @brief Функция помещает в выходную строку все операторы из стека пока не встретит открытую скобку или стек не окажется пустым
  @param t_node_oper Указатель на текущий head
  @param polish_str Выходная строка с обратной польской записью
  @param count_of_open_bracket Количество открытых скобок в выражении на текущий момент
  @return Статус ошибки : 0 - работа завершилась корректно, 1 - работа завершилась с ошибкой "Не корректное выражение"
*/
...
/*!
  @brief Функция помещает в выходную строку функцию, если та была в выражении перед открытой скобкой
  @param t_node_oper Указатель на текущий head
  @param polish_str Выходная строка с обратной польской записью
  @param count_of_open_bracket Количество открытых скобок в выражении на текущий момент
...
/*!
  ...
  @return Статус ошибки : 0 - работа завершилась корректно, 2 - работа завершилась с ошибкой "Ошибка калькуляции"
*/
